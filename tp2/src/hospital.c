...
/*
 * Recibe un pokemon y lo libera de memoria junto a su nombre
*/
...
/*
 * Reserva memoria para una estructura entrenador y la inicializa
 * con su nombre e id.
 * 
 * Devuelve Null en caso de error.
*/
entrenador_t* entrenador_crear(char* nombre, size_t id) {
  entrenador_t* entrenador = malloc(sizeof(entrenador_t));
  if (!entrenador) return NULL;

  entrenador->nombre = malloc(strlen(nombre) + 1);
...
/*
 * Recibe un entrenador y lo libera de memoria junto a su nombre
*/
bool entrenador_destruir(entrenador_t* entrenador, void* dato) {
...
/*
 * Funcion que compara dos pokemones por su nombre, devuelve mayor a 0 si el primero es
...
hospital_t* hospital_crear() {
  ...
  nuevo_hospital->entrenadores = lista_crear();

  if (!nuevo_hospital->pokemones || !nuevo_hospital->entrenadores) {
    lista_destruir(nuevo_hospital->entrenadores);
    abb_destruir(nuevo_hospital->pokemones);
...
pokemon_t* pokemon_crear(char* nombre, size_t nivel, entrenador_t* entrenador) {
  ...
  nuevo_pokemon->entrenador = entrenador;

  return nuevo_pokemon;
}

/**
 * Recibe un vector de pokemones con su cantidad, el entrenador, el vector de strings con los 
 * pokemones ingresados y la cantidad de nuevos pokemones (todos validados previamente)
 * 
 * Agrega los nuevos pokemones a su abb.
 * 
 * Si la reserva de momemoria falla devuelve NULL, caso contrario devuelve el puntero del
 * vector agrandado
 */
abb_t* agregar_entrenador_y_pokemones(abb_t* pokemones, entrenador_t* entrenador,
                                      char** ingreso_entrenador, size_t cant_nuevos_pokemones) {
  ...
  for (size_t i = 0; i < cant_nuevos_pokemones; i++) {
    pokemon_t* nuevo_pokemon = pokemon_crear(ingreso_entrenador[ingreso_entrenador_posicion],
                                             (size_t)atoi(ingreso_entrenador[ingreso_entrenador_posicion + 1]),
                                             entrenador);
    if (!nuevo_pokemon) return NULL;

    if (!abb_insertar(pokemones, nuevo_pokemon)) {
      free(nuevo_pokemon);
      return NULL;
    }
...
/**
 * Recibe un hospital valido, un vector de strings del ingreso de pokemones de un entrenador 
 * validado con su cantidad de elementos
 * 
 * La funcion se ocupa de interpretar el formato del string y almacenar los datos del entreador
 * y de sus pokemones en el hospital devolviendo el hospital actualizado o NULL en caso de error
 * reservando memoria
 */
hospital_t* hospital_agregar_entrenador(hospital_t* hospital, char** ingreso_entrenador,
                                        size_t cantidad_elementos_leida) {
  ...
  entrenador_t* entrenador = entrenador_crear(nombre_entrenador, id_entrenador);
  if (!entrenador) return NULL;

  lista_t* entrenadores_aux = lista_insertar(hospital->entrenadores, entrenador);
  if (!entrenadores_aux) {
    printf("Error al insertar el entrenador en la lista\n");
    free(entrenador);
    return NULL;
  }

  hospital->entrenadores = entrenadores_aux;

  abb_t* pokemones_aux =
      agregar_entrenador_y_pokemones(hospital->pokemones, entrenador, ingreso_entrenador,
                                     cant_nuevos_pokemones);
  if (!pokemones_aux) {
    return NULL;
  }
...
/**
 * la funcion recibe el vector de strings valido con los datos del ingreso de pokemones por un 
 * entrenador leido del archivo al cual va a liberar de memoria, tambien recibe un bool que marca 
 * si libera los nombres de los pokemones de la memoria o no (si no hay errores previos se prevee
 * que los punteros de strings se reutilizen en el hospital y no hay que liberarlos en esta instancia)
 */
...
bool hospital_leer_archivo(hospital_t* hospital, const char* nombre_archivo) {
  ...
  while (linea) {
    ...
    // par (id y nombre del entrenador o nombre y nivel de pokemones), el archivo esta mal y se
    // devuelve false liberando todo antes
    ...
    hospital_t* hospital_aux =
        hospital_agregar_entrenador(hospital, ingreso_entrenador, cantidad_elementos_leida);
    if (!hospital_aux) {
      eliminar_informacion_ingreso_entrenador(ingreso_entrenador, true);
      fclose(archivo);
      return false;
...
void hospital_destruir(hospital_t* hospital) {
  ...
  lista_con_cada_elemento(hospital->entrenadores, (funcion_a_elemento)entrenador_destruir, NULL);
